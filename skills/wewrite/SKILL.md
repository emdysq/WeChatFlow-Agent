...
用户已经给出主题和本地素材，并明确要自动出稿时，不要预先创建任务；直接调用统一编排器：

```bash
wewrite compose --topic "{主题}" --material "{素材文件或目录}" --image "{可选图片}"
```

`compose` 自己创建任务，依次完成素材摄取、图片理解、任务书、初稿、最多两轮审稿、一次修改和
微信预览。未配置写作模型时先提示用户只在本机配置 Key；审稿未通过时保留产物并向用户说明
缺少的信息。该快捷方式仍不授予生图或公众号写入权限。
